...
class multi_task_model(PretrainedBartModel):
    base_model_prefix = "model"
...
#         self.sent_linear_layser = nn.Linear(1024, 2)
        self.sent_linear_layser_1 = nn.Linear(1024, 512)
        self.sent_linear_layser_2 = nn.Linear(512, 256)
        self.sent_linear_layser_3 = nn.Linear(256, 128)
        self.sent_linear_layser_4 = nn.Linear(128, 64)
        self.sent_linear_layser_5 = nn.Linear(64, 32)
        self.sent_linear_layser_6 = nn.Linear(32, 2)
...
#             outputs = (masked_lm_loss,) + outputs
            # 新增的代码
            sent_emb = torch.index_select(encoder_outputs, 1, torch.squeeze(sent_ids)).cuda()
            sent_logits = self.sent_linear_layser_1(sent_emb)
            sent_logits = self.sent_linear_layser_2(sent_logits)
            sent_logits = self.sent_linear_layser_3(sent_logits)
            sent_logits = self.sent_linear_layser_4(sent_logits)
            sent_logits = self.sent_linear_layser_5(sent_logits)
            sent_logits = self.sent_linear_layser_6(sent_logits)
            sent_loss = loss_fct(torch.squeeze(sent_logits), ext_label.view(-1))
            loss = masked_lm_loss+sent_loss*0.1
            print(sent_loss)
            outputs = (loss,) + outputs
        return outputs
...
class BartForSequenceClassification(PretrainedBartModel):
    ...
    Returns:
        ...
        if labels is not None:  # prepend loss to output,
            masked_lm_loss = F.cross_entropy(logits.view(-1, self.config.num_labels), labels.view(-1))
